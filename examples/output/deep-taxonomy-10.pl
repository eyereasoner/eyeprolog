...
answer(2,
       ['X0' = check3,
        'X1' = "C3 OK - the chain reaches the midpoint n5 and still carries both side-label branches."]).
answer(2,
       ['X0' = check4, 'X1' = "C4 OK - the final taxonomy step from n9 to n10 was completed."]).
answer(2,
       ['X0' = check5, 'X1' = "C5 OK - once n10 is reached, the terminal class a2 is derived."]).
answer(2,
       ['X0' = check6,
...
answer(3,
       ['X0' = report,
        'X1' = "The test succeeds: starting from one individual classified as n0, the rules eventually classify it as n10 and then as a2."]).
query(4, reason(_0, _1), ['X0' = _0, 'X1' = _1]).
result(4, complete, 1).
answer(4,
       ['X0' = report,
        'X1' = "The adjacent rules mirror the Eyeling N3 deep-taxonomy-10 chain: each rule advances one taxonomy level and adds the matching side labels."]).
